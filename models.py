...
class GraphEncoder(nn.Module):
    ...
    def forward(self, batch):
        x = F.relu(self.node_linear(batch.x.float()))
        edge_attr = F.relu(self.edge_linear(batch.edge_attr.float()))

        x = F.relu(self.conv1(x, batch.edge_index, edge_attr))
        x = F.relu(self.conv2(x, batch.edge_index, edge_attr))
        x = F.relu(self.conv3(x, batch.edge_index, edge_attr))
...
class GraphToTextConditionalGeneration(nn.Module):
    def __init__(self, node_in_dim, edge_in_dim, vocab_size, bos_token_id, eos_token_id, pad_token_id):
        ...
        self.decoder_config = GPT2Config(
            ...
            n_positions=1024,
            n_embd=770,          # Hidden dimensionality
            n_layer=10,          # Reduced from 12 to 10 to hit ~100M parameter target
        ...
        self.graph_encoder = GraphEncoder(node_in_dim, edge_in_dim, hidden_dim=2048, out_dim=decoder_hidden_dim)
