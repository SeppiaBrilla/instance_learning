...
def generate_graph(i, num_vertices, edge_probability, num_colors):
    stdout = subprocess.run([f'minizinc ./data/mzn_models/graph_coloring.mzn ./data/graph_coloring_instances/graph_coloring_{i}.dzn --solver cp-sat'], shell=True, stdout=subprocess.PIPE).stdout.decode()
    sat =  not "=====UNSATISFIABLE=====" in stdout
    
    subprocess.run([f'minizinc ./data/mzn_models/graph_coloring.mzn ./data/graph_coloring_instances/graph_coloring_{i}.dzn --solver gecode -c --no-output-ozn --fzn .cache/graph_coloring_{i}.fzn'], shell=True)
    subprocess.run([f'python ../flatzinc_parser/flatzinc_parser.py .cache/graph_coloring_{i}.fzn ./data/graph_coloring_graphs/graph_coloring_{i}.graph'], shell=True)
    subprocess.run([f'rm .cache/graph_coloring_{i}.fzn'], shell=True)
    with open(f"./data/graph_coloring_graphs/graph_coloring_{i}.graph", 'r+') as f:
        content = f.read()
        f.seek(0, 0)
